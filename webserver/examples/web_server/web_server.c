...
typedef DWORD(WINAPI * GetModuleFileNameExFuncPtr)(_In_      HANDLE hProcess,
    _In_opt_  HMODULE hModule,
    _Out_     LPTSTR lpFilename,
    _In_      DWORD nSize
    );
GetModuleFileNameExFuncPtr getProcessImageFileNamePtr = NULL;
...
static int client_process_id = -1;
static char client_process_name[PATH_MAX];
...
static void process_command_line_arguments(char *argv[], char **options) {
  ...
  if (argv[1] == NULL || memcmp(argv[1], "-psn_", 5) != 0) {
    ...
    for (i = cmd_line_opts_start; argv[i] != NULL; i += 2) {
      ...
      if (!strcmp(argv[i], "-client_process_id"))
          client_process_id = atoi(argv[i + 1]);
      else if (!strcmp(argv[i], "-client_process_name"))
          strcpy(client_process_name, argv[i + 1]);
      else
...
static int find_client_process() {

#ifdef _WIN32    
    int i = 0;
    HANDLE Handle = OpenProcess( PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, client_process_id );
    if (Handle)
    {
        char buffer[MAX_PATH];
        if ((*getProcessImageFileNamePtr)(Handle, 0, buffer, MAX_PATH))
        {
            while (buffer[i] != 0)
            {
            ...
            return !strcmp(buffer, client_process_name) ? 1 : 0;
        }        
        CloseHandle(Handle);
    }
#endif
...
static DWORD WINAPI check_client_alive(void * param)
{
    while (exit_flag == 0) {
        if (client_process_id != -1 && !find_client_process())        
            exit_flag = -1;
        Sleep(3000);
    }
...
#ifdef _WIN32
  HMODULE hm = LoadLibraryA("psapi.dll");
  getProcessImageFileNamePtr = (GetModuleFileNameExFuncPtr)GetProcAddress(hm, "GetModuleFileNameExA");
  DWORD thid;
  HANDLE th = CreateThread(NULL, 0, &check_client_alive, NULL, 0, &thid);
#endif
  init_server_name();
